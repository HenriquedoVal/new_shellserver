...
__declspec(dllexport) char *ss_echo(const char *msg);
...
__declspec(dllexport) char *ss_get_prompt(const char *path, short term_width, int error_code, unsigned cmd_dur_ms);

__declspec(dllexport) char *ss_get_path(const char *refpath);
__declspec(dllexport) bool ss_add_refpath(const char *path, /*nullable*/ const char *as);
...
__declspec(dllexport) char *ss_get_all_refpaths(void);
