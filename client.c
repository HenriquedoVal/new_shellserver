...
char *ss_get_prompt(const char *path, short term_width, int error_code, unsigned cmd_dur_ms) {
...
char *ss_get_path(const char *refpath) {
    if (g_ctx == NULL && !ss_init()) return NULL;
